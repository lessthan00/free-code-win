# Gemini Native API Support & Startup Script Overhaul

## Summary
Native Gemini Interactions API support via a fetch adapter, plus a unified startup script (`start.ps1`) with provider-aware model selection. Users can now choose API provider → select models under that API → launch, all from a single menu.

---

## Key Changes

### 1. Gemini Fetch Adapter (`src/services/api/gemini-fetch-adapter.ts`) — NEW
- **Anthropic → Gemini request translation**: `POST https://generativelanguage.googleapis.com/v1beta/interactions?alt=sse`
- **Message mapping**: `user`→`user_input`, `assistant`→`model_output`, `tool_use`→`function_call`, `tool_result`→`function_result`
- **Tool translation**: Anthropic `input_schema` → Gemini `parameters` + `type: "function"` wrapper
- **Streaming SSE translation**: `interaction.created`→`message_start`, `step.start`→`content_block_start`, `step.delta`→`content_block_delta`, `interaction.completed`→`message_delta`+`message_stop`
- **Model mapping**: opus→gemini-3.1-pro-preview, sonnet→gemini-3.5-flash, haiku→gemini-3.1-flash-lite
- Graceful error handling with Anthropic-format error responses

### 2. Provider Architecture Updates

| File | Change |
|------|--------|
| `src/utils/model/providers.ts` | Added `'gemini'` to `APIProvider` union type and `getAPIProvider()` detection chain |
| `src/services/api/client.ts` | Added Gemini client creation block (between Codex and firstParty), reads `GEMINI_API_KEY` |
| `src/utils/model/configs.ts` | Added `gemini:` field to all 14 model config constants |
| `src/utils/managedEnvConstants.ts` | Registered `CLAUDE_CODE_USE_GEMINI` and `GEMINI_API_KEY` in both managed and safe env var sets |

### 3. start.ps1 Overhaul
- **Provider-aware model selection**: Menu [2] dynamically shows only the current provider's models
- **Data-driven**: `$ProviderModels` hashtable with 7 providers, each with model names + descriptions
- **Gemini provider [7]**: Collects and persists `GEMINI_API_KEY`, sets `CLAUDE_CODE_USE_GEMINI=1`
- **DeepSeek enhancement**: Models get `[1m]` context suffix, thinking mode enabled
- **OpenAI enhancement**: API Key input option alongside existing OAuth
- **Auto-setup**: Bun detection/install, dependency install, build menu, global command registration
- **Merged**: All `start-bulit.ps1` functionality merged in, redundant file deleted

### 4. Provider Isolation
Each provider is selected via mutually exclusive `CLAUDE_CODE_USE_*` flags. The Gemini adapter only activates when `CLAUDE_CODE_USE_GEMINI=1`; it does not affect DeepSeek (firstParty path), Anthropic, OpenAI/Codex, Bedrock, Vertex, or Foundry.

### 5. Build Verification
- `bun run build:dev:full` — 5689 modules bundled, zero type errors
- Output: `./cli-dev.exe`
